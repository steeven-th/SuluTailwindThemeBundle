...
| `--iw-card-shadow-color` | the variant's, then `--iw-cards-shadow-color`, then `#000` | Colour the shadow is drawn in, three levels deep: this override, then the variant of the block the card sits in, then the site-wide colour from **Components > Cards**. That last one matters for cards outside any block - an article listing page carries no variant. The opacity is applied with `color-mix()`, the colour arriving as a hex or a palette reference and carrying no alpha of its own. |
