...
* **Site-wide cards**: configure surface, title/text/badge colors, title and text sizes, border (width + style), padding, image ratio and composable hover effects (card transform, image effect, shadow, border color, duration, easing) from the admin **Components → Cards** section (applies to every card)
...
otherwise surface as a broken admin build in someone else's project. It needs a
parser, which CI installs and you can install too:

```bash
npm install --no-save @babel/parser
```

Without it the test **skips**, so a contributor with no JavaScript toolchain
does not get a red suite over a check they cannot run - and CI still has the
guarantee.

Note the plain `npm install` at the root rather than through
`public/js/package.json`: that one declares peer dependencies on React and
sulu-admin-bundle, which npm would try to resolve for a check that needs
neither.
